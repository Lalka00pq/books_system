...
# ... (rest of the file remains, I will just append the missing endpoints at the end by using a broader replacement)

# --- User's Personal Collection (The "Entity" requirements) ---
...
# --- Global Catalog (Protected by Admin Role for mutations) ---

def role_required(role: UserRole):
    def dependency(current_user: User = Depends(get_current_user)):
        if current_user.role != role:
            raise AppException(403, "FORBIDDEN", f"Operation requires {role.value} role")
        return current_user
    return dependency
...
async def update_catalog_book(
    ...
    stmt = update(Book).where(Book.id == id).values(**book_data.model_dump()).returning(Book)
...
async def patch_catalog_book(
    ...
    stmt = update(Book).where(Book.id == id).values(**update_data).returning(Book)
    res = await db.execute(stmt)
    await db.commit()
    return res.scalar_one()
