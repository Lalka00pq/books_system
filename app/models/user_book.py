...
class UserBook(BaseModel):
    ...
    status: Mapped[BookStatus] = mapped_column(Enum(BookStatus), default=BookStatus.PLAN_TO_READ, nullable=False)
    added_at: Mapped[DateTime] = mapped_column(DateTime(timezone=True), server_default=func.now())

    user: Mapped["User"] = relationship("User", back_populates="books")
    book: Mapped["Book"] = relationship("Book", back_populates="user_associations")
